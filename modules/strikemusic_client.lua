...
local function rawGet(url)
    ...
    return response.Body, nil, response.Headers or {}
end
...
function StrikeMusicClient.Create(Api, Storage)
    ...
    local function getResponseHeader(headers, headerName)
        local expectedName = tostring(headerName or ""):lower()

        for name, value in pairs(headers or {}) do
            if tostring(name):lower() == expectedName then
    ...
    local function isDirectMediaResponse(sourceUrl, mediaType, body, headers)
        local contentType = getResponseHeader(headers, "content-type")
        local path = tostring(sourceUrl or ""):lower():match("^[^?]+") or ""
        local expectedExtension = mediaType == "mp4" and ".mp4" or ".mp3"
        local expectedPrefix = mediaType == "mp4" and "video/" or "audio/"
        local isExpectedContentType = contentType:find(expectedPrefix, 1, true) ~= nil
        local isGenericBinary = contentType:find("application/octet-stream", 1, true) ~= nil
        local isExpectedExtension = path:sub(-#expectedExtension) == expectedExtension

        if contentType:find("text/html", 1, true) then
            return false, "direct_media_content_invalid"
        end

        if isExpectedContentType or (isGenericBinary and isExpectedExtension) then
            return true, nil
        end

        if type(body) == "string" and body:sub(1, 32):lower():find("<html", 1, true) then
            return false, "direct_media_content_invalid"
        end

        return false, "direct_media_type_mismatch"
    end

    function client.DownloadDirectResult(player, result, mediaType)
        local ready = ensureReady()
        ...
        local sourceUrl = tostring(job.source_url or "")

        if not sourceUrl:match("^https?://") then
            Api.FailPersonalMusicDownload(player, job.download_job_id, "direct_media_url_required")
            return {
                status = "blocked",
                reason = "direct_media_url_required",
                job = job
            }
        end

        local body, downloadError, headers = rawGet(sourceUrl)
        ...
        local isMedia, mediaReason = isDirectMediaResponse(
            sourceUrl,
            mediaType,
            body,
            headers
        ...
        if not isMedia then
            Api.FailPersonalMusicDownload(player, job.download_job_id, mediaReason)
            return {
                status = "blocked",
                reason = mediaReason,
                job = job
        ...
        if not completeResult or completeResult.status ~= "completed" then
            return {
                status = "blocked",
                reason = completeResult and completeResult.reason or "download_complete_failed",
                metadata = metadataResult.metadata,
                complete = completeResult
            }
        end
